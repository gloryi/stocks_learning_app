...
class textMorfer():
    ...
    def morf_text(self, text):
        # Case morfer
        random.seed(self.seed)
        text = "".join(_ if random.randint(0,20) < 15 else _.upper() for _ in text)
        text = "".join(_ if random.randint(0,20) < 15 else _.lower() for _ in text)
        #text = "".join(_ if random.randint(0,30) < 29 else random.choice(self.glith_glyphs) for _ in text)
        text = "".join(_ if random.randint(0,30) < 29 else random.choice(self.alphabet[_.lower()]) if _.lower() in self.alphabet else _ for _ in text)
        text = "".join(_ if random.randint(0,10) < 5 else random.choice(self.alphabet_2[_]) if _ in self.alphabet_2 else _ for _ in text)
