...
class UpperLayout:
    def __init__(S, display_instance):
        ...
        S.utf_font = backend.api().font.Font(CHINESE_FONT, 150)
        S.utf_font1 = backend.api().font.Font(CHINESE_FONT, 100)
        S.utf_font2 = backend.api().font.Font(CHINESE_FONT, 50)
        S.combo = 1
        S.tiling = ""
        S.tiling_utf = True
    ...
    def set_image(S, path_to_image, minor=False):
        ...
        if not path_to_image:
            ...
            if not minor:
                ...
                S.image_minor = backend.api().transform.flip(
                    S.images_cached[path_to_image], random.choice([True, False]), False
                )
        else:
            if not minor:
                S.image = S.images_cached[path_to_image]
    ...
    def redraw(S):
        ...
        if S.show_mode or S.blink:
            ...
            for i, x in enumerate(range(100, W, 500)):
                for j, y in enumerate(range(100, H, 500)):
                    ...
                    S.place_text(
                        S.tiling_text + f" {S.tiling_cooldown//5}",
                        x,
                        y,
                        transparent=True,
                        renderer=S.utf_font2,
                        base_col=(
                            clip_color(225 + S.variation * 4),
                            225 - S.variation,
        ...
        interpolate = lambda col1, col2, percent: (
            inter_color(col1[0], col2[0], S.timing_ratio),
        ...
        if not S.blink:
            line_color = interpolate(
                colors.col_active_lighter, col2, 1.0 - S.timing_ratio
            )
        else:
        ...
        backend.api().draw.rect(
            S.display_instance,
            line_color,
            ((W // 2 - ((W // 4) * (S.percent)) / 2), 45, (W // 4 * S.percent), 20),
        )
