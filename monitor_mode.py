...
class ChainedEntity:
    def __init__(self, chained_feature, features_chain, chains, pygame_instance, W, H):
    ...
    def register_keys(self, key_states, time_percent, time_based=False):
        ...
        if time_based:
            ...
            if self.done:
                time_p = (time_p - self.time_perce_reserved) / (
                    1.0 - self.time_perce_reserved
                )
    ...
    def register_idle_mouse(self):
        ...
        if self.mode == "QUESTION":
            ...
            self.idle_coursor = self.question_pxls_to_price(
                int(H / STAKE_PART) * self.active_index_float
            )
    ...
    def produce_candles(self):
        if self.mode == "QUESTION":
            ...
            return self.chained_feature.get_candles_with_offset(
                self.active_index, VISUAL_PART
            )
    ...
    def produce_lines(self):
        if self.mode == "QUESTION":
            ...
            return self.chained_feature.get_lines_with_offset(
                self.active_index, VISUAL_PART
            )
    ...
    def variate(self):
        ...
        if self.variation == 0:
            if self.mode == "QUESTION":
                avaliable_numbers = [
                    _ for _ in range(11) if _ not in self.constant_variations
                ]
...
class ChainedDrawer:
    def __init__(self, pygame_instance, display_instance, W, H):
        ...
        self.cyrillic_120 = self.pygame_instance.font.Font(
            CYRILLIC_FONT, 120, bold=True
        )
    ...
    def draw_line(self, line):
        ...
        for geometry in geometries:
            ...
            if geometry.rect:
                ...
                self.pygame_instance.draw.rect(
                    self.display_instance, (50, 50, 50), (x, y, w, h), width=2
                )
    ...
    def generateOCHLPicture(self, line):
        ...
        def drwLineSimple(X1, Y1, X2, Y2):
            self.pygame_instance.draw.line(
                self.display_instance, (180, 180, 180), (X1, Y1), (X2, Y2), 1
            )
        ...
        def drwZonBrdr(zone):
            ...
            dY = Y2 - Y1
            self.pygame_instance.draw.rect(
                self.display_instance, (180, 180, 180), (Y1, X1, (dY), (dX)), width=1
            )
        ...
        def drwSqrZon(zone, x1, y1, x2, y2, col, width=0):
            try:
                ...
                self.pygame_instance.draw.line(
                    self.display_instance, lighter_col, (Y1, X1), (Y1, X2), 2
                )
                self.pygame_instance.draw.line(
                    self.display_instance, lighter_col, (Y1, X2), (Y2, X2), 2
                )
                self.pygame_instance.draw.line(
                    self.display_instance, lighter_col, (Y2, X2), (Y2, X1), 2
                )
                self.pygame_instance.draw.line(
                    self.display_instance, lighter_col, (Y2, X1), (Y1, X1), 2
                )
                self.pygame_instance.draw.rect(
        ...
        def drawCircleInZon(zone, x1, y1, r, col, width=0):
            try:
                ...
                X1 = int(X + dx * x1)
                Y1 = int(Y + dy * y1)
                self.pygame_instance.draw.circle(
                    self.display_instance, col, (Y1, X1), r, width=width
                )
            except Exception as e:
                pass
        ...
        def drwLineZon(zone, x1, y1, x2, y2, col, strk=1):
            try:
                ...
                self.pygame_instance.draw.line(
                    self.display_instance, lighter_col, (Y1, X1), (Y2, X2), strk + 1
                )
                self.pygame_instance.draw.line(
                    self.display_instance, col, (Y1, X1), (Y2, X2), strk
                )
        ...
        def drawCandle(
            ...
            if entry and entry > _l and entry < _h:
                h_position, l_position = (
                    (1 - hwick - candle_len // 8, 1 - body_mid_zone)
                    if entry > body_mid
                    else (1 - body_mid_zone, 1 - lwick + candle_len // 8)
                )
                if not draw_special:
                    drwSqrZon(
                        zone,
        ...
        def find_suitable_squares(candles, minV, maxV):
            ...
            selected_rects = []
            for front in range(initial_index, 0, -8):
                i2 = front
                i1 = max(0, front - 8)
                ...
                if not max_high_abs - H // 8 < 0:
                    ...
                    selected_rects.append([mnh, w1 + wd * 0.40, mh, w1 + wd * 0.60])
        ...
        if line.mode == "QUESTION" and not line.burn_mode and line.idle_x:
            ...
            drwLineZon(
                draw_tasks[0][1], 1, line.idle_x, 0, line.idle_x, line_color, strk=strk
            )
            drwLineZon(
                draw_tasks[0][1],
                1 - idle_l,
        ...
        if line.mode != "QUESTION":
            for special_line in lines:
                for i, (p1, p2) in enumerate(zip(special_line[:-1], special_line[1:])):
                    ...
                    x2 = (p2[0] - o1 + 0.5) / DPTH
                    rgb_col = interpolate(
                        colors.white, colors.col_black, abs(line.variation / 40)
                    )
                    drwLineZon(
                        draw_tasks[0][1], 1 - y1, x1, 1 - y2, x2, rgb_col, strk=2
                    )
        if True:
            ...
            for i, (p1, p2) in enumerate(zip(high_tf_line[:-1], high_tf_line[1:])):
                ...
                x0 = (p1[0] - 2) / (
                    DPTH * (len(high_tf_line) / VISUAL_PART) / 0.3
                ) + 0.35
                x1 = (p1[0] + 0.5) / (
                    DPTH * (len(high_tf_line) / VISUAL_PART) / 0.3
                ) + 0.35
...
class KeyboardChainModel:
    ...
    def get_inputs(self):
        ...
        for control_key in self.mapping:
            if keys[control_key]:
                self.mapping[control_key] = self.process_button(
                    self.mapping[control_key], self.down
                )
            else:
                self.mapping[control_key] = self.process_button(
                    self.mapping[control_key], self.up
                )
...
class ChainedProcessor:
    def __init__(
        self, pygame_instance, display_instance, data_label, data_path, beat_time=1
    ):
        self.W = W
        self.H = H
        self.producer = ChainedsProducer(data_label, data_path)
        ...
        self.active_entity = ChainedEntity(
            self.producer.produce_next_feature(),
    ...
    def add_line(self):
        ...
        return (
            self.active_entity.time_estemated,
            self.producer.produce_meta(),
            self.producer.produce_meta_minor(),
        )
    ...
    def process_inputs(self, time_elapsed=0):
        ...
        if self.active_entity:
            self.active_entity.register_keys(
                pressed_keys,
