...
def get_candles(asset_name, index):
    ...
    for candle in knwon_prices[asset_name][
        int(index) - (VISUAL_PART - GENERATION_TIME_SIZE):
    ]:
        yield candle
...
def get_dense(asset_name, index):
    ...
    initial_ind = max(
        0,
        (int(index) - (VISUAL_PART - GENERATION_TIME_SIZE) + VISUAL_PART)
        - VISUAL_PART * HIGHER_TIMEFRAME_SCALE,
    )
    range_selector = filter(lambda _: _.index >=
                            initial_ind, dense_prices[asset_name])
...
class ChainedFeature:
    def __init__(S, source, start_point):
        ...
        S.dense_candles = S.pre_process_candles(
            get_dense(source, start_point), only_visual=True
        )

        S.hints = [False for _ in range(5)]
        random.seed(S.start_point)
        S.hint_candles = [random.choice(S.candles).index for _ in range(5)]
        for c in S.candles:
            if c.index in S.hint_candles:
                c.is_hint = True
        ...
        S.rised = False
        S.attached_image = ""
        S.basic_timing_per_level = {0: 45, 1: 45, 2: 45}
        ...
        for candle in candles_generator:
            ...
            if candles and len(candles) < VISUAL_PART:
                ...
                if upper_prev <= upper_next and lower_prev >= lower_next:
                    candle.pierce = True
                    candle.upper_pierce_line = max(
                        candles[-1].o, candles[-1].c)
                    candle.lower_pierce_line = min(
                        candles[-1].o, candles[-1].c)
            ...
            if candle.overhigh:
                if not last_high or not high_low_line:
                    ...
                    if len(high_low_line) > 6:
                        if (
                            high_low_line[-4][1] > high_low_line[-6][1]
                            and high_low_line[-4][1] > high_low_line[-2][1]
                        ):
                            del high_low_line[-4]
                ...
                if last_high or not high_low_line:
                    ...
                    if len(high_low_line) > 6:
                        if (
                            high_low_line[-4][1] < high_low_line[-6][1]
                            and high_low_line[-4][1] < high_low_line[-1][1]
                        ):
                            del high_low_line[-4]
                            high_low_line[-4].append(2)
    ...
    def check_hints(S):
        ...
        print(S.hint_candles)
        print(S.hints)
        for i in range(len(S.hint_candles)):
            if not S.hints[i]:
                return False
...
class ChainedModel:
    ...
    def get_next_feature(S):
        ...
        S.burning_in_work = list(
            filter(lambda _: _.is_burning, S.burning_in_work))
    ...
    def set_burning_in_work(S):
        if S.is_burning():
            S.burning_in_work, S.burning_chain = (
                S.burning_chain[: S.burning_size],
                S.burning_chain[S.burning_size:],
            )
